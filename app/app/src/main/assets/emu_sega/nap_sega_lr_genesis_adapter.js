...
var BUILD='BUILD2NC_SEGA_RESTORE_SKIN_DIRECT_ROM_STAGE16';
...
function patchEngineOriginGuards(sourceLabel, code){
  ...
  var patches=0;
  function mark(m){ patches++; return 'console.warn(\"NAP BUILD2NC origin guard neutralized: '+m.replace(/[\\\"\n\r]/g,' ')+'\"); void 0;'; }
  ...
  if(guardCtor.test(code)){ guardCtor.lastIndex=0; code=code.replace(guardCtor,function(m){ patches++; return '(console.warn(\"NAP BUILD2NC origin Error ctor neutralized\"),null)'; }); }

  // BUILD2NC safety: previous BUILD2MW could create `console.warn(...)var`
  ...
  if(hitPhrase || patches){ log('ORIGIN GUARD + DOCUMENT CURRENTSCRIPT PATCH '+sourceLabel+' phrase='+(hitPhrase?'YES':'NO')+' patches='+patches+' bytesBefore='+before+' bytesAfter='+code.length); }
  else { log('ORIGIN GUARD + DOCUMENT CURRENTSCRIPT PATCH '+sourceLabel+' no explicit origin-guard phrase found'); }
  return code;
}
function evalEngineClosure(sourceLabel, code){
  ...
  // BUILD2NC: do NOT pass Object.create(document) into the engine. DOM methods
  ...
  var currentScriptPatchCount=0;
  clean=clean.replace(/\bdocument\.currentScript\b/g,function(){ currentScriptPatchCount++; return 'window.__napSegaCurrentScriptShim'; });
  try{ global.__napSegaCurrentScriptShim={src:safeSrc,parentNode:document.head, getAttribute:function(n){ return n==='src'?safeSrc:null; }}; }catch(_shimErr){}
  log('CURRENT_SCRIPT TOKEN PATCH '+sourceLabel+' replacements='+currentScriptPatchCount+' realDocument=YES safe='+safeSrc);
  var runner=null;
  try{
    runner=new Function('window','document','__napSourceLabel',
      ...
      'var getComputedStyle=window.getComputedStyle ? window.getComputedStyle.bind(window) : undefined;\n'+
      'var atob=window.atob ? window.atob.bind(window) : undefined; var btoa=window.btoa ? window.btoa.bind(window) : undefined;\n'+
  ...
  try{
    var exported=runner(global,document,sourceLabel);
...
function loadOnlineEngine(){
  ...
  drawCoreScreen('ONLINE ENGINE PROBE','Stahuji lrusso Genesis.min.js jako text. BUILD2NC ho spusti pres closure eval, ne pres iframe.', 'boot');
...
function bootSameOriginRunner(romBuffer, info, reason){
  prepareMount();
  ...
  try{
    ...
    liveIframe=frame;
    frame.title='AtariHelp Sega BUILD2NC same-origin runner';
    ...
    function sendRom(){
      ...
      try{
        ...
        log('BUILD2NC RUNNER ROM POSTED title='+title+' size='+romBuffer.byteLength+' reason='+(reason||'direct'));
      }catch(e){
        log('BUILD2NC RUNNER ROM POST FAILED '+(e.message||String(e)));
        throw e;
      }
    }
    function onMsg(ev){
      var d=ev.data||{};
      if(!d.napSega) return;
      log(String(d.msg||d.type||'RUNNER_MESSAGE'));
      if(d.type==='ready') sendRom();
      if(d.type==='started'){ running=true; hideOverlay(); showToast('SEGA runner hlasi START. Zkus START/A/B/C.'); }
      if(d.type==='failed'){ showToast('SEGA runner selhal: '+String(d.msg||'')); }
    }
    global.addEventListener('message',onMsg,false);
    frame.onload=function(){ log('BUILD2NC RUNNER IFRAME LOAD - cekam READY a posilam ROM'); setTimeout(sendRom,800); };
    mountEl.appendChild(frame);
    var mdVideo=byId('mdVideo'); if(mdVideo) mdVideo.style.display='none';
    hideOverlay();
    showToast('BUILD2NC: zkousim same-origin runner bez moji grafiky.',4200);
    log('BUILD2NC RUNNER FALLBACK START reason='+(reason||'none')+' title='+title);
    return true;
  }catch(e){
    restoreCanvasFallback();
    drawCoreScreen('RUNNER FAILED', e.message||String(e), 'error');
    log('BUILD2NC RUNNER FALLBACK FAILED '+(e.message||String(e)));
    throw e;
  }
}
...
function bootWithEmbedGenesis(romBuffer, info){
  ...
  try{
    // BUILD2NC: first call inside the same global lexical context that owns
    ...
    var ret=null;
    try{ ret=global.eval('(typeof embedGenesis===\"function\" ? embedGenesis : window.embedGenesis)(window.__napSegaEmbedOpts)'); }
    catch(evalCallErr){
      log('embedGenesis EVAL-CALL FAILED '+(evalCallErr.message||String(evalCallErr))+'; retry bound window call');
      ret=embedGenesisFn.call(global,opts);
    }
    ...
    if(/Illegal invocation|Invocation/i.test(emsg)){
      log('BUILD2NC: Illegal invocation pri primem volani - spoustim same-origin runner fallback bez moji konzolove grafiky.');
  ...
  try{
    setTimeout(function(){
      if(!running){
        var gm=byId('genesisMount');
        var canv=gm ? gm.getElementsByTagName('canvas').length : 0;
        var kids=gm ? gm.children.length : 0;
        log('embedGenesis POSTCHECK running='+running+' mountChildren='+kids+' canvases='+canv+' status='+adapter.getStatus());
      }
    },2500);
  }catch(_postErr){}
  return true;
...
var adapter={
  ...
  loadRom:function(romBytes, info){
    ...
    return ensureEngine().then(function(engineStatus){
      ...
      log('BUILD2NC: iframe fallback vypnuty; vlastni konzolova grafika vypnuta; pokracuji jen realny core call + native browser binding probe.');
