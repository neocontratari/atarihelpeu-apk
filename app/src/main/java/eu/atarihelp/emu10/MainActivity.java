...
public class MainActivity extends Activity {
    ...
    private volatile int nativeAudioGeneration = 0; // BUILD2RO: kills stale AudioTrack threads after every ROM change; prevents cumulative slowdown.
    ...
    private volatile String nativePerformanceMode = "HIGH"; // BUILD2RO: SBIRKA selects HIGH QUALITY / LOW PERFORMANCE.
    private volatile long nativeLastSaveLogAtMs = 0; // BUILD2RO: one tap must not create 4 log files.
    ...
    private String buildNativeInPlaceLog() {
        ...
        out.append("SEGA C++ IN-PLACE LOG / BUILD2RO\n");
        out.append("AtariHelp.eu EMU-10 BUILD2RO_SEGA_NATIVE_CPP_ONLY_ZEROED_REFERENCE_AUDIO_STAGE131\n\n");
        ...
        out.append("- Sega emulace je v BUILD2RO C++ only; Java/WebView wrapper se nespousti; C++ CORE UI tlacitko je odstranene; FM/PSG zeroed reference audio bez FM/PSG desync dropu; SBIRKA prepina LOW/HIGH vykon; mobile visibility/orientation nesmi vypnout native video; C++ region respektuje ROM header; nulove recty se ignoruji.\n");
        return out.toString();
    }
    ...
    private synchronized void startNativeCoreAudioStream() {
        if (nativeCoreAudioRun || (nativeCoreAudioThread != null && nativeCoreAudioThread.isAlive()) || nativeCurrentAudioTrack != null) {
            appendNativeLog("NATIVE_AUDIO_STREAM_PRESTART_HARD_STOP_RO_ZEROED_REFERENCE_AUDIO_QT oldGen=" + nativeAudioGeneration);
            stopNativeCoreAudioStream();
        }
        final int audioGen = ++nativeAudioGeneration; // BUILD2RO: one valid AudioTrack writer per ROM session.
        ...
        nativeCoreAudioThread = new Thread(() -> {
            ...
            final int chunk = 384; // BUILD2RO: stable AudioTrack clock; stereo frames; clean FM/PSG frontend output.
            AudioTrack track = null;
            try {
                int min = AudioTrack.getMinBufferSize(sampleRate, AudioFormat.CHANNEL_OUT_STEREO, AudioFormat.ENCODING_PCM_16BIT);
                // BUILD2RO: keep the AudioTrack clock stable. Bigger buffer hides short FM music stalls without changing emulation speed.
                int wantedBytes = chunk * 2 * 2 * 16;
                ...
                appendNativeLog("NATIVE_AUDIO_STREAM_START_RO_ZEROED_REFERENCE_AUDIO_QT gen=" + audioGen + " sampleRate=" + sampleRate + " chunk=" + chunk + " minBytes=" + min + " bufferBytes=" + bufferBytes + " setFrames=" + setFrames + " mode=" + nativePerformanceMode + " perfLowLatency=" + (Build.VERSION.SDK_INT >= 26));

                int prefilled = 0;
                int preLoops = 0;
                long preDeadline = System.currentTimeMillis() + 500;
                while (nativeCoreAudioRun && audioGen == nativeAudioGeneration && prefilled < Math.min(6144, chunk * 2 * 8) && System.currentTimeMillis() < preDeadline) {
                    int got = 0;
                    try { got = NativeSegaCoreBridge.pullAudioStereo(pcm, chunk); }
                    catch (Throwable pullErr) { appendNativeLog("NATIVE_AUDIO_PREFILL_PULL_ERROR_RO " + safeMsg(pullErr)); got = 0; }
                    if (got > 0) {
                ...
                if (!nativeCoreAudioRun || audioGen != nativeAudioGeneration) {
                    appendNativeLog("NATIVE_AUDIO_START_CANCELLED_RO staleGen=" + audioGen + " current=" + nativeAudioGeneration);
                    return;
                }
                track.play();
                appendNativeLog("NATIVE_AUDIO_PREFILL_RO_ZEROED_REFERENCE_AUDIO_QT gen=" + audioGen + " frames=" + prefilled + " loops=" + preLoops + " playState=" + track.getPlayState());
                int loops = 0;
                int underrunLoops = 0;
                while (nativeCoreAudioRun && audioGen == nativeAudioGeneration) {
                    int got = 0;
                    try { got = NativeSegaCoreBridge.pullAudioStereo(pcm, chunk); }
                    catch (Throwable pullErr) { appendNativeLog("NATIVE_AUDIO_PULL_ERROR_RO " + safeMsg(pullErr)); got = 0; }
                    if (got <= chunk / 4) { underrunLoops++; }
                    if (loops < 16 || loops % 180 == 0) appendNativeLog("NATIVE_AUDIO_PULL_RO_ZEROED_REFERENCE_AUDIO_QT gen=" + audioGen + " got=" + got + " loop=" + loops + " underrunLoops=" + underrunLoops + " mode=" + nativePerformanceMode);
                    // BUILD2RO: always feed AudioTrack a full clock chunk. Native writes real samples and controlled decay only on real underrun;
                ...
                appendNativeLog("NATIVE_AUDIO_STREAM_ERROR_RO gen=" + audioGen + " " + safeMsg(t));
            } finally {
                ...
                appendNativeLog("NATIVE_AUDIO_STREAM_STOP_RO_ZEROED_REFERENCE_AUDIO_QT gen=" + audioGen + " current=" + nativeAudioGeneration);
            }
        }, "AtariHelpSegaReferenceAudioRO_" + audioGen);
    ...
    private synchronized void stopNativeCoreAudioStream() {
        final int stopGen = ++nativeAudioGeneration; // BUILD2RO: invalidate stale AudioTrack writers before starting another ROM.
        ...
        if (at != null) {
            ...
            // BUILD2RO: on S8 old AudioTrack.write() could stay blocked after ROM change.
        ...
        if (t != null && t.isAlive() && Thread.currentThread() != t) {
            ...
            if (t.isAlive()) appendNativeLog("NATIVE_AUDIO_THREAD_STILL_ALIVE_RO invalidatedGen=" + stopGen + " releasedTrack=YES will exit on generation guard");
        }
        if (nativeCoreAudioThread == t) nativeCoreAudioThread = null;
        appendNativeLog("NATIVE_AUDIO_STREAM_STOP_REQUEST_RO_ZEROED_REFERENCE_AUDIO_QT gen=" + stopGen + " hardReleaseTrack=" + (at != null));
    }
        ...
        Runnable r = () -> {
            try {
                if (old != null) {
                    ...
                    try { if (old.getParent() instanceof ViewGroup) ((ViewGroup) old.getParent()).removeView(old); } catch (Throwable t) { appendNativeLog("NATIVE_VIEW_REMOVE_UI_ERR_RO " + safeMsg(t)); }
                }
                appendNativeLog("NATIVE_VIEW_REMOVE_UI_OK_RO reason=" + reason);
            } catch (Throwable t) {
                appendNativeLog("NATIVE_VIEW_REMOVE_UI_FAIL_RO reason=" + reason + " " + safeMsg(t));
            }
        };
    ...
    private synchronized String stopNativeInPlaceHard(String reason) {
        ...
        nativeRomLoadGeneration++; // BUILD2RO: cancel stale delayed audio/render watchdogs when leaving Sega/Atari/VBXE.
    ...
    public class AHNative {
            ...
            try {
                ...
                ui.post(() -> {
                    try {
                        ...
                        // BUILD2RO: enableInPlace is now idempotent. Rotation/resize/visibility on S8 called it many times;
                        ...
                        if (nativeInPlaceView != null && nativeInPlaceView.getParent() == rootFrame) {
                            ...
                            appendNativeLog("NATIVE_VIEW_REUSE_RO parent=rootFrame noRecreate=YES");
                            return;
                        }
                        if (nativeInPlaceView != null) {
                            ...
                            nativeInPlaceView = null;
                            appendNativeLog("NATIVE_VIEW_RECREATE_RO oldViewRemoved=YES detachedOrBadParent");
                        }
                        ...
                        // BUILD2RO: never show native view before JS sends a valid monitor/landscape rect.
                        ...
                        appendNativeLog("NATIVE_VIEW_REATTACH_RO parent=rootFrame freshView=YES hiddenUntilValidRect=YES");
                        nativeInPlaceEnabled = true;
            ...
            try {
                ui.post(() -> {
                    try {
                        ...
                        // BUILD2RO: Samsung/WebView can send a transient 0x0 rect when returning from landscape.
                        // Ignoring that rect prevents the native video from becoming black after portrait return.
                        if (w < 120 || h < 80) { appendNativeLog("SET_RECT_SKIP_SMALL_RO x=" + x + " y=" + y + " w=" + w + " h=" + h); return; }
                        ...
                        if (landscapeFull && web != null) {
                            // BUILD2RO: on real phones the C++ native view is the video layer, WebView is only transparent controls/log.
                            ...
                            appendNativeLog("NATIVE_Z_ORDER_RO landscapeWebControlsOverNative=YES topLabelsHidden=YES");
                        } else {
                        ...
                        appendNativeLog("SET_RECT_OK_RO x=" + Math.max(0, x) + " y=" + Math.max(0, y) + " w=" + ww + " h=" + hh + " visible=" + nativeInPlaceEnabled + " landscapeFull=" + landscapeFull);
        ...
        public String loadRomBase64(String name, String b64) {
            try {
                ...
                // BUILD2RO: audio starts only after visible native frame; zeroed reference audio/music timing lives in C++ core.
                ...
                appendNativeLog("FRESH_ROM_GENERATION_RO gen=" + loadGen + " oldAudioGen=" + nativeAudioGeneration + " oldDraw=" + nativeViewDrawCounter);
                stopNativeCoreAudioStream();
                try { NativeSegaCoreBridge.shutdown(); appendNativeLog("FRESH_ROM_HARD_STOP_BEFORE_LOAD_RO gen=" + loadGen); } catch (Throwable ignored) {}
                ...
                appendNativeLog("REAL_CORE_RENDER_ACTIVE_RO after ROM load audio=WAIT_FRAME_AND_VIEW_DRAW");
                forceNativeViewRedrawBurst("afterRomLoad_RM");
        ...
        public String setPerformanceMode(String mode) {
            try {
                ...
                appendNativeLog("NATIVE_PERF_MODE_RO mode=" + nativePerformanceMode + " sdk=" + Build.VERSION.SDK_INT + " " + nativePerf);
                if (nativeInPlaceView != null) {
                    try { nativeInPlaceView.forceRedrawOnce(); } catch (Throwable ignored) {}
                }
                return "NATIVE_PERF_MODE_OK_RO mode=" + nativePerformanceMode + " " + nativePerf;
            } catch (Throwable t) {
                return "NATIVE_PERF_MODE_ERROR_RO " + safeMsg(t);
            }
        }
        ...
        public String saveLog() {
            try {
                ...
                if (now - nativeLastSaveLogAtMs < 2200) {
                    appendNativeLog("SAVE_LOG_DEDUP_RO ignored deltaMs=" + (now - nativeLastSaveLogAtMs));
                    return "SAVE_LOG_DEDUP_RM";
                }
                nativeLastSaveLogAtMs = now;
                String fn = "AtariHelp_SEGA_CPP_INPLACE_LOG_BUILD2RO_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new Date()) + ".txt";
    ...
    private void forceNativeViewRedrawBurst(final String reason) {
        ...
        // BUILD2RO: keep only a short burst; long delayed bursts from old ROMs added work after multiple ROM loads.
    ...
    private void scheduleNativeAudioAfterFrameAndViewDraw(final String romName, final byte[] romData, final int gen, final int attempt) {
        ...
        ui.postDelayed(() -> {
            try {
                if (gen != nativeRomLoadGeneration) { appendNativeLog("NATIVE_AUDIO_WAIT_CANCELLED_RO staleGen=" + gen + " current=" + nativeRomLoadGeneration); return; }
                ...
                appendNativeLog("NATIVE_AUDIO_WAIT_FRAME_VIEW_RO attempt=" + attempt + " gen=" + gen + " hasFrame=" + hasFrame + " viewReady=" + viewReady + " draw=" + nativeViewDrawCounter + "/" + nativeViewDrawCounterAtRomLoad + " " + (st == null ? "null" : st.replace('\n',' ').substring(0, Math.min(430, st.length()))));
                if (hasFrame && viewReady) {
                    startNativeCoreAudioStream();
                    appendNativeLog("NATIVE_AUDIO_START_AFTER_FRAME_VIEW_RO name=" + safeFileName(romName) + " gen=" + gen + " audioProfile=QT_ZEROED_REFERENCE_AUDIO");
                    return;
                }
                ...
                stopNativeCoreAudioStream();
                nativeLastStatus = "NATIVE_AUDIO_NOT_STARTED_NO_VISIBLE_FRAME_RO name=" + safeFileName(romName);
                appendNativeLog(nativeLastStatus);
                forceNativeViewRedrawBurst("audioNoVisibleFrame_RM");
            } catch (Throwable t) {
                appendNativeLog("NATIVE_AUDIO_WAIT_FRAME_VIEW_RO_ERROR " + safeMsg(t));
            }
        }, delay);
    }
    ...
    private void scheduleNativeRenderWatchdog(final String romName, final byte[] romData, final int gen, final int attempt) {
        ...
        ui.postDelayed(() -> {
            try {
                if (gen != nativeRomLoadGeneration) { appendNativeLog("NATIVE_RENDER_WATCHDOG_CANCELLED_RO staleGen=" + gen + " current=" + nativeRomLoadGeneration); return; }
                String st = NativeSegaCoreBridge.realCoreStatus();
                boolean hasFrame = st != null && st.indexOf("frameReady=YES") >= 0 && st.indexOf("frameCounter=0") < 0;
                boolean viewReady = nativeInPlaceEnabled && nativeInPlaceView != null && nativeInPlaceView.getParent() != null
                        && nativeInPlaceView.getVisibility() == View.VISIBLE && nativeViewDrawCounter > nativeViewDrawCounterAtRomLoad;
                appendNativeLog("NATIVE_RENDER_WATCHDOG_RO attempt=" + attempt + " gen=" + gen + " hasFrame=" + hasFrame + " viewReady=" + viewReady + " draw=" + nativeViewDrawCounter + "/" + nativeViewDrawCounterAtRomLoad + " " + (st == null ? "null" : st.replace('\n',' ').substring(0, Math.min(520, st.length()))));
                if (hasFrame && viewReady) {
                    forceNativeViewRedrawBurst("watchdogFrameViewOK_RM");
                    return;
                }
                if (attempt <= 1 && romData != null && romData.length > 0 && nativeInPlaceEnabled) {
                    appendNativeLog("NATIVE_RENDER_WATCHDOG_FRESH_RELOAD_RO name=" + safeFileName(romName) + " bytes=" + romData.length + " gen=" + gen);
                    ...
                    nativeLastStatus = "NATIVE_RENDER_WATCHDOG_RELOAD_RO " + (reload == null ? "null" : reload.replace('\n',' '));
                    appendNativeLog(nativeLastStatus.substring(0, Math.min(900, nativeLastStatus.length())));
                    forceNativeViewRedrawBurst("watchdogFreshReload_RM");
                ...
                if (!hasFrame || !viewReady) {
                    stopNativeCoreAudioStream();
                    nativeLastStatus = "NATIVE_RENDER_NO_VISIBLE_FRAME_AUDIO_STOPPED_RO name=" + safeFileName(romName);
                    appendNativeLog(nativeLastStatus);
                    forceNativeViewRedrawBurst("watchdogNoVisibleFrameAudioStopped_RM");
                }
            } catch (Throwable t) {
                appendNativeLog("NATIVE_RENDER_WATCHDOG_RO_ERROR " + safeMsg(t));
            }
        }, delay);
    }
    ...
    private class NativeInPlaceView extends TextureView implements TextureView.SurfaceTextureListener {
        ...
        private void startRenderThreadIfReady(String reason) {
            ...
            renderThread.start();
            appendNativeLog("NATIVE_TEXTURE_THREAD_START_RO reason=" + reason + " view=" + getWidth() + "x" + getHeight());
        }
        ...
        private void renderLoop(String reason) {
            ...
            while (running) {
                ...
                try {
                    ...
                    appendNativeLog("NATIVE_TEXTURE_RENDER_ERROR_RO " + safeMsg(t));
                } finally {
                    try { if (c != null) unlockCanvasAndPost(c); } catch (Throwable t) { appendNativeLog("NATIVE_TEXTURE_UNLOCK_ERROR_RO " + safeMsg(t)); }
                }
                long cost = System.nanoTime() - startNs;
                nativeLastRenderCostNs = cost;
                // BUILD2RO: SBIRKA performance switch. HIGH keeps full-speed render for Nox/new phones.
            ...
            appendNativeLog("NATIVE_TEXTURE_THREAD_STOP_RO reason=" + reason + " drawCounter=" + nativeViewDrawCounter + " lastCostMs=" + (nativeLastRenderCostNs / 1000000.0));
        }
        ...
        private void drawTextureFrame(Canvas canvas) {
            ...
            try {
                ...
                if (nativeLandscapeFullVideo) {
                    // BUILD2RO: never crop the 320x224 game image. User reported Sonic lives/HUD cut off in landscape.
                ...
                if (nowMs - nativeRenderPerfWindowStartMs >= 5000) {
                    appendNativeLog("NATIVE_RENDER_PERF_RO frames5s=" + nativeRenderPerfWindowFrames + " slow22ms=" + nativeRenderPerfSlowFrames + " lastCostMs=" + (nativeLastRenderCostNs / 1000000.0) + " android=" + Build.VERSION.SDK_INT + " perfMode=" + nativePerformanceMode + " landscapeFull=" + nativeLandscapeFullVideo);
                ...
                if (nativeViewDrawCounter <= 4 || nativeViewDrawCounter % 300 == 0) {
                    appendNativeLog("NATIVE_TEXTURE_FRAME_RO count=" + nativeViewDrawCounter + " view=" + w + "x" + h + " gameDst=" + dst.toShortString() + " perfMode=" + nativePerformanceMode + " landscapeFull=" + nativeLandscapeFullVideo + " costMs=" + (nativeLastRenderCostNs / 1000000.0));
                }
            } catch (Throwable t) {
                try { canvas.drawColor(Color.rgb(20, 0, 0)); } catch (Throwable ignored) {}
                appendNativeLog("NATIVE_TEXTURE_FRAME_ERROR_RO " + safeMsg(t));
            }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // BUILD2RO: WebView must be transparent in landscape; HTML is controls-only over native C++ video.
