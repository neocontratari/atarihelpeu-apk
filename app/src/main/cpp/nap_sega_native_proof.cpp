...
// BUILD2RT: hard region/memory audit path built on the first clean zeroed FM/PSG frontend.
...
// BUILD2RT: hard audit + memory-safe fixed audio rings.
...
// BUILD2RT: Android 9/S8 needs a no-starve audio reservoir and less native video capture work.
...
static std::atomic<int> g_audio_mono_mode{0}; // BUILD2RT: audit switch for old/mobile phones; Java may request mono AudioTrack.
...
// BUILD2RT values use the core output directly: the real fix is zeroing ClownMDEmu += buffers,
...
static void nap_audio_clock_governor_sleep() {
    // BUILD2RT: audio-master no-starve guard.
...
static int nap_audio_pull_stereo(jshort *out, int stereoFrames) {
    ...
    for (int i = 0; i < stereoFrames; ++i) {
        ...
        if (hfl || hfr || hp) {
            ...
            // BUILD2RT: do not invent a bass/rumble filter here. The previous RN/RM/RL filters
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_REGION_MEMORY_AUDIT_RT sonic_main fixedAudioClock=YES zeroInputBuffers=YES coreLPF=ON noRumbleFilter=YES noSamplePick=YES singleAudioPath=YES PURE_CPP_SEGA_PATH=YES JAVA_SEGA_WRAPPER_RUNNING=NO WEB_AUDIO_RUNNING=NO LEGACY_JAVA_AUDIO_THREAD=NO nativeAudioRegistry=YES audioMasterClock=AUDIT noHardTrim=YES audioMode=" << (nap_audio_mono_mode() ? "MONO" : "STEREO") << " audioChannels=" << (nap_audio_mono_mode() ? 1 : 2) << " fmGain=100 psgGain=100 masterGain=90 target=" << nap_audio_target_fifo()
        ...
        << " audioRingAudit=FIXED_RING_NO_DEQUE ringCapacity=65536 noRuntimeAudioAllocs=YES regionMemoryAudit=YES javaMixGuard=YES"
...
// BUILD2RT: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2RT/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2RT: tiny Android frontend for the real ClownMDEmu-core.
...
static std::mutex g_frame_mutex; // BUILD2RT: render must not wait on full ClownMDEmu_Iterate()
static bool g_real_core_loaded_but_render_guarded = false;
static uint32_t g_guard_frame_counter = 0;
static int g_real_step_stage = 0; // BUILD2RT: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2RT: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2RT: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_real_scanline_rendered(void *user_data, cc_u16f scanline, const cc_u8l *pixels,
    ...
    // BUILD2RT HARD AUDIT: do NOT shift the active region to x=0.
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    // BUILD2RT CRITICAL: ClownMDEmu FM_OutputSamples uses += into sample_buffer.
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // BUILD2RT: zeroed native YM/FM buffer, then average-resample to 48 kHz. No stale samples.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    // BUILD2RT CRITICAL: PSG_Update also uses +=. Zero the scratch buffer before generation.
    std::fill(tmp.begin(), tmp.end(), 0);
    out.clear();
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // BUILD2RT: zeroed PSG buffer first; PSG_Update uses +=, so clearing is mandatory. Then box-average downsample.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2RT: PCM is diagnostic only for now; Sonic DAC is inside YM/FM, not Mega-CD PCM.
    static thread_local std::vector<cc_s16l> tmp;
...
static std::string nap_real_detect_and_set_region(const uint8_t* bytes, size_t size) {
    ...
    if ((hasE && !hasU) || isJue) {
        ...
        g_real_cfg_region_label = std::string(isJue ? "PAL_EU_50_OVERSEAS reason=JUE_APP_DEFAULT_EU " : "PAL_EU_50_OVERSEAS reason=ROM_E ")
            + "header=" + upper + " videoHz=50 frameNs=20000000 z80ClockMode=PAL fmClockMode=PAL psgClockMode=PAL";
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2RT: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2RT: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_REGION_MEMORY_AUDIT_RT stereo/mono AudioTrack; PURE_CPP audit; zeroed callback buffers before ClownMDEmu += mixing; PAL/NTSC auto timing logged; framebuffer stride proof; no Java wrapper/audio fallback; no fake rumble filter; no FM/PSG desync drops";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2RT\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2RT: renderer asks for 320x224. Copy directly under the frame mutex and avoid
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2RT real core worker start gen=%d bigstack=8MB", generation);
    ...
    try {
        ...
        while (g_real_thread_run.load() != 0 && generation == g_real_thread_generation.load()) {
            ...
            if (g_real.loaded) {
                // BUILD2RT: LOW/HIGH is video-only. HIGH captures every frame; LOW presents ~30 fps.
                ...
                // BUILD2RT: S8 no-starve mode reduces native scanline capture when audio FIFO is low.
                ...
                if ((it & 31u) == 0u) {
                    ...
                    // BUILD2RT: keep lock order clean. Do not take audio mutex while holding real mutex.
                    size_t audioBacklogForStatus = nap_audio_min_fifo();
                    std::lock_guard<std::mutex> lock(g_real_mutex);
                    g_real.status = "REAL_CORE_RENDER_OK_WORKER_THREAD_RT frameCounter=" + std::to_string(g_real.frame_counter) +
            ...
            if (nap_mobile_no_starve() && nap_audio_min_fifo() < nap_audio_low_water() && g_real.loaded) {
                // BUILD2RT: no-starve catchup. If S8 AudioTrack is hungry, do not wait for the next
            ...
            if (now + frame_period < next_tick) {
                ...
                // BUILD2RT: RF-style clock guard. If phone is late, resync; do not run extra frames that make Sonic rhythm drift.
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2RT real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2RT: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RT NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
                    ...
                    "Status: C++ only Sega runtime; Java wrapper disabled; QT audio profile + RT region/memory hard-audit zeroed-buffer clock governor; no ROM in APK";
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2RT ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2RT ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_configureRuntime(JNIEnv* env, jclass, jint sdk, jint cores, jstring model) {
    ...
    std::ostringstream out;
    out << "NATIVE_RUNTIME_OK_RT sdk=" << sdk << " cores=" << cores << " model=" << smodel
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_setAudioMode(JNIEnv* env, jclass, jstring mode) {
    ...
    std::ostringstream out;
    out << "NATIVE_AUDIO_MODE_OK_RT mode=" << (mono ? "MONO" : "STEREO")
        << " channels=" << (mono ? 1 : 2)
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_setPerformanceMode(JNIEnv* env, jclass, jstring mode) {
    ...
    std::ostringstream out;
    out << "NATIVE_PERF_MODE_OK_RT mode=" << (low ? "LOW" : "HIGH")
...
// BUILD2RT: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_buildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RT NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    ...
                    "C++ PCM audio generator: OK\n"
                    "C++ guarded render: OK\n"
                    "Status: normal Sega UI; Java wrapper disabled; RT region/memory hard-audit zeroed-buffer clock governor + native visible-frame guard; no ROM in APK";
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_renderPattern(JNIEnv* env, jclass, jint width, jint height, jint frame, jintArray argbOut) {
    ...
    if (argbOut && width > 0 && height > 0) {
        ...
        int needed = width * height;
        if (len >= needed) {
            // BUILD2RT: mobile video fast path. Do not allocate a std::vector every frame and do not copy twice.
...
// BUILD2RT REAL CORE ADAPTER SLOT
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_shutdown(JNIEnv* env, jclass) {
    ...
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RT_QT_CPP_ONLY worker=STOPPED audio=CLEARED input=RELEASED");
#else
    for (int i = 0; i < 8; ++i) g_input[i] = false;
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RT_QT_CPP_ONLY noVendor input=RELEASED");
#endif
}
