...
// BUILD2RO: C++ only + lower latency FM/PSG clean audio.
...
// BUILD2RO: zeroed reference audio for ClownMDEmu.
...
// BUILD2RO values use the core output directly: the real fix is zeroing ClownMDEmu += buffers,
...
static int nap_audio_pull_stereo(jshort *out, int stereoFrames) {
    ...
    for (int i = 0; i < stereoFrames; ++i) {
        ...
        if (hfl || hfr || hp) {
            ...
            // BUILD2RO: do not invent a bass/rumble filter here. The previous RN/RM/RL filters
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_ZEROED_FRONTEND_RO sonic_main fixedAudioClock=YES stereo=YES zeroInputBuffers=YES coreLPF=ON noRumbleFilter=YES noSamplePick=YES fmGain=100 psgGain=100 masterGain=90 target=" << nap_audio_target_fifo()
...
// BUILD2RO: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2RO/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2RO: tiny Android frontend for the real ClownMDEmu-core.
...
static std::mutex g_frame_mutex; // BUILD2RO: render must not wait on full ClownMDEmu_Iterate()
static bool g_real_core_loaded_but_render_guarded = false;
static uint32_t g_guard_frame_counter = 0;
static int g_real_step_stage = 0; // BUILD2RO: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2RO: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2RO: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_real_scanline_rendered(void *user_data, cc_u16f scanline, const cc_u8l *pixels,
    ...
    for (int x = 0; x < sw; ++x) {
        // BUILD2RO: respect active horizontal boundaries and hide the long-standing bad left edge columns.
        int sx = x;
    ...
    if (y >= sh - 1) {
        ...
        // BUILD2RO: worker writes into frame_argb, renderer reads only display_argb.
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    // BUILD2RO CRITICAL: ClownMDEmu FM_OutputSamples uses += into sample_buffer.
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // BUILD2RO: zeroed native YM/FM buffer, then average-resample to 48 kHz. No stale samples.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    // BUILD2RO CRITICAL: PSG_Update also uses +=. Zero the scratch buffer before generation.
    std::fill(tmp.begin(), tmp.end(), 0);
    out.clear();
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // BUILD2RO: zeroed PSG buffer first; PSG_Update uses +=, so clearing is mandatory. Then box-average downsample.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2RO: PCM is diagnostic only for now; Sonic DAC is inside YM/FM, not Mega-CD PCM.
    static thread_local std::vector<cc_s16l> tmp;
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2RO: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2RO: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_ZEROED_FRONTEND_RO stereo AudioTrack; ZEROED callback buffers before ClownMDEmu += mixing; core low-pass ON; no fake rumble filter; no FM/PSG desync drops";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2RO\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2RO: renderer asks for 320x224. Copy directly under the frame mutex and avoid
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2RO real core worker start gen=%d bigstack=8MB", generation);
    try {
        ...
        while (g_real_thread_run.load() != 0 && generation == g_real_thread_generation.load()) {
            ...
            if (g_real.loaded) {
                // BUILD2RO: LOW/HIGH is video-only. HIGH captures every frame; LOW presents ~30 fps.
                ...
                if ((it & 31u) == 0u) {
                    ...
                    std::lock_guard<std::mutex> lock(g_real_mutex);
                    g_real.status = "REAL_CORE_RENDER_OK_WORKER_THREAD_RO frameCounter=" + std::to_string(g_real.frame_counter) +
            ...
            if (now + frame_period < next_tick) {
                ...
                // BUILD2RO: RF-style clock guard. If phone is late, resync; do not run extra frames that make Sonic rhythm drift.
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2RO real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2RO: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RO NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
                    ...
                    "Status: C++ only Sega runtime; Java wrapper disabled; QT audio profile + RO zeroed-buffer reference audio; no ROM in APK";
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2RO ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2RO ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_setPerformanceMode(JNIEnv* env, jclass, jstring mode) {
    ...
    std::ostringstream out;
    out << "NATIVE_PERF_MODE_OK_RO mode=" << (low ? "LOW" : "HIGH")
...
// BUILD2RO: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_buildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RO NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    ...
                    "C++ PCM audio generator: OK\n"
                    "C++ guarded render: OK\n"
                    "Status: normal Sega UI; Java wrapper disabled; RO zeroed-buffer reference audio + native visible-frame guard; no ROM in APK";
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_renderPattern(JNIEnv* env, jclass, jint width, jint height, jint frame, jintArray argbOut) {
    ...
    if (argbOut && width > 0 && height > 0) {
        ...
        int needed = width * height;
        if (len >= needed) {
            // BUILD2RO: mobile video fast path. Do not allocate a std::vector every frame and do not copy twice.
...
// BUILD2RO REAL CORE ADAPTER SLOT
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_shutdown(JNIEnv* env, jclass) {
    ...
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RO_QT_CPP_ONLY worker=STOPPED audio=CLEARED input=RELEASED");
#else
    for (int i = 0; i < 8; ++i) g_input[i] = false;
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RO_QT_CPP_ONLY noVendor input=RELEASED");
#endif
}
