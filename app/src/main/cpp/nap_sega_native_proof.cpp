...
// BUILD2QP: FM + LIGHT PSG clean audio.
...
static const size_t NAP_AUDIO_TARGET_FIFO = 2048;      // ~43 ms @ 48k: porad nizka latence, mene underrun/chrapot na S8/A12.
static const size_t NAP_AUDIO_MAX_FIFO = 4096;         // ~85 ms hard cap; porad daleko od stare 0.5s ozveny.
static const size_t NAP_AUDIO_DESYNC_LIMIT = 768;      // keep FM/PSG queues aligned; Sonic ring/jump must not slide behind music.
...
static int nap_audio_pull_mono(jshort *out, int frames) {
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        if (hf || hp) {
            got++;
            // Sonic: FM music + PSG jump/ring. QP keeps headroom and adds a small de-click LPF.
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_CLEAN_QP sonic_main target=" << NAP_AUDIO_TARGET_FIFO
...
// BUILD2QP: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2QP/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2QP: tiny Android frontend for the real ClownMDEmu-core.
...
static int g_real_step_stage = 0; // BUILD2QP: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2QP: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2QP: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_fm_acc >= srcRate) {
            ...
            // BUILD2QP: leave headroom for a light PSG path; avoid 2QL clipping/chrceni.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_psg_acc >= srcRate) {
            ...
            // BUILD2QP: Sonic jump/ring effects live here. Keep audible, but much softer than QL to stop crackle.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2QP: PCM is diagnostic only for now. QL mixed PCM/CDDA and made audio strange/crackly.
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2QP: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2QP: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_CLEAN_QP to Java AudioTrack; de-click underrun fade; low-pass enabled; PCM diagnostic only; precise 60Hz worker clock; anti-lag FIFO trim enabled";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2QP\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2QP: never call ClownMDEmu_Iterate() from Android View.onDraw.
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2QP real core worker start gen=%d bigstack=8MB", generation);
    try {
        ...
        while (g_real_thread_run.load() != 0 && generation == g_real_thread_generation.load()) {
            ...
            auto iter_start = std::chrono::steady_clock::now();
            {
                ...
                if (g_real.loaded) {
                    ...
                    if ((it & 31u) == 0u) {
                        g_real.status = "REAL_CORE_RENDER_OK_WORKER_THREAD_QP frameCounter=" + std::to_string(g_real.frame_counter) +
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2QP real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2QP: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2QP NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2QP ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2QP ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
// BUILD2QP: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_buildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2QP NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    ...
                    "C++ PCM audio generator: OK\n"
                    "C++ guarded render: OK\n"
                    "Status: normal Sega UI; real ClownMDEmu-core big-stack pthread render + EU/PAL region auto + FM/PSG QP clean audio + re-enter path; no ROM in APK";
    return env->NewStringUTF(s.c_str());
}
...
// BUILD2QP REAL CORE ADAPTER SLOT
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_shutdown(JNIEnv* env, jclass) {
    ...
    nap_audio_clear();
    {
        ...
        g_real.loaded = false;
        g_real.frame_ready = false;
        g_real.status = "REAL_CORE_STOPPED_BY_LIFECYCLE_QP";
    }
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_QP worker=STOPPED audio=CLEARED input=RELEASED");
#else
    for (int i = 0; i < 8; ++i) g_input[i] = false;
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_QP noVendor input=RELEASED");
#endif
}
