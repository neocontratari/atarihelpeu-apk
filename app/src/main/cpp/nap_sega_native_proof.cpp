...
// BUILD2QW: C++ only + lower latency FM/PSG clean audio.
...
static const size_t NAP_AUDIO_TARGET_FIFO = 2048;      // BUILD2QW: navrat k QP clean profilu; QR bylo chraplavejsi.
static const size_t NAP_AUDIO_MAX_FIFO = 4096;         // BUILD2QW: QP rezerva proti underrunu na S8/A12; stale tvrdý cap proti 0.5s ozvene.
static const size_t NAP_AUDIO_DESYNC_LIMIT = 768;      // BUILD2QW: QP alignment; PSG ring/jump zustane slyset, ale nesjede za FM.
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_QT_AUDIO_KEEP_BLACKSCREEN_GUARD_QW sonic_main target=" << NAP_AUDIO_TARGET_FIFO
...
// BUILD2QW: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2QW/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2QW: tiny Android frontend for the real ClownMDEmu-core.
...
static int g_real_step_stage = 0; // BUILD2QW: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2QW: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2QW: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_fm_acc >= srcRate) {
            ...
            // BUILD2QW: leave headroom for a light PSG path; avoid 2QL clipping/chrceni.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_psg_acc >= srcRate) {
            ...
            // BUILD2QW: Sonic jump/ring effects live here. Keep audible, but much softer than QL to stop crackle.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2QW: PCM is diagnostic only for now. QL mixed PCM/CDDA and made audio strange/crackly.
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2QW: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2QW: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_QT_AUDIO_KEEP_BLACKSCREEN_GUARD_QW to Java AudioTrack; Java wrapper disabled; QP-clean profile restored; render watchdog prevents sound-only black screen";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2QW\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2QW: never call ClownMDEmu_Iterate() from Android View.onDraw.
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2QW real core worker start gen=%d bigstack=8MB", generation);
    try {
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2QW real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2QW: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2QW NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2QW ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2QW ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
// BUILD2QW: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
...
// BUILD2QW REAL CORE ADAPTER SLOT
