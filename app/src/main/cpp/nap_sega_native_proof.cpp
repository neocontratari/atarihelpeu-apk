...
// BUILD2RN: C++ only + lower latency FM/PSG clean audio.
...
// BUILD2RN: reference audio cleanup for ClownMDEmu.
...
static const int NAP_AUDIO_STARTUP_MUTE_FRAMES = 2048;
...
// RN values restore audible FM lead. Bass cleanup is handled by noCoreLPF + rumbleBlock,
// not by simply making music quieter.
static const int NAP_FM_GAIN_PERCENT = 100;
static const int NAP_PSG_GAIN_PERCENT = 48;
static const int NAP_MASTER_GAIN_PERCENT = 96;
...
static int nap_audio_pull_stereo(jshort *out, int stereoFrames) {
    ...
    for (int i = 0; i < stereoFrames; ++i) {
        ...
        if (hfl || hfr || hp) {
            ...
            l = nap_audio_rumble_block(l, g_audio_rumble_l);
            r = nap_audio_rumble_block(r, g_audio_rumble_r);
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_REFERENCE_FRONTEND_RN sonic_main fixedAudioClock=YES stereo=YES boxAverageResampler=YES antiAliasPSG=YES noSamplePick=YES startupMute=2048 noCoreLPF=YES rumbleBlock=YES fmGain=100 psgGain=48 masterGain=96 target=" << nap_audio_target_fifo()
...
// BUILD2RN: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2RN/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2RN: tiny Android frontend for the real ClownMDEmu-core.
...
static std::mutex g_frame_mutex; // BUILD2RN: render must not wait on full ClownMDEmu_Iterate()
static bool g_real_core_loaded_but_render_guarded = false;
static uint32_t g_guard_frame_counter = 0;
static int g_real_step_stage = 0; // BUILD2RN: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2RN: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2RN: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_real_scanline_rendered(void *user_data, cc_u16f scanline, const cc_u8l *pixels,
    ...
    for (int x = 0; x < sw; ++x) {
        // BUILD2RN: respect active horizontal boundaries and hide the long-standing bad left edge columns.
        int sx = x;
    ...
    if (y >= sh - 1) {
        ...
        // BUILD2RN: worker writes into frame_argb, renderer reads only display_argb.
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // RN: average-resample native YM/FM to 48 kHz. No sample picking, no fake bass clamp.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // RN: PSG is ~223 kHz. Picking one sample out of five aliases into fake bass/gong.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2RN: PCM is diagnostic only for now; Sonic DAC is inside YM/FM, not Mega-CD PCM.
    static thread_local std::vector<cc_s16l> tmp;
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2RN: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2RN: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_REFERENCE_FRONTEND_RN stereo AudioTrack; core low-pass OFF; rumble blocker; box-average resampler; no sample-pick bass/gong; no FM/PSG desync drops";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2RN\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2RN: renderer asks for 320x224. Copy directly under the frame mutex and avoid
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2RN real core worker start gen=%d bigstack=8MB", generation);
    try {
        ...
        while (g_real_thread_run.load() != 0 && generation == g_real_thread_generation.load()) {
            ...
            if (g_real.loaded) {
                // BUILD2RN: LOW/HIGH is video-only. HIGH captures every frame; LOW presents ~30 fps.
                ...
                if ((it & 31u) == 0u) {
                    ...
                    std::lock_guard<std::mutex> lock(g_real_mutex);
                    g_real.status = "REAL_CORE_RENDER_OK_WORKER_THREAD_RN frameCounter=" + std::to_string(g_real.frame_counter) +
            ...
            if (now + frame_period < next_tick) {
                ...
                // BUILD2RN: RF-style clock guard. If phone is late, resync; do not run extra frames that make Sonic rhythm drift.
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2RN real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2RN: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RN NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
                    ...
                    "Status: C++ only Sega runtime; Java wrapper disabled; QT audio profile + RN reference audio cleanup; no ROM in APK";
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2RN ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2RN ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_setPerformanceMode(JNIEnv* env, jclass, jstring mode) {
    ...
    std::ostringstream out;
    out << "NATIVE_PERF_MODE_OK_RN mode=" << (low ? "LOW" : "HIGH")
...
// BUILD2RN: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_buildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2RN NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    ...
                    "C++ PCM audio generator: OK\n"
                    "C++ guarded render: OK\n"
                    "Status: normal Sega UI; Java wrapper disabled; RN reference audio cleanup + native visible-frame guard; no ROM in APK";
    return env->NewStringUTF(s.c_str());
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_renderPattern(JNIEnv* env, jclass, jint width, jint height, jint frame, jintArray argbOut) {
    ...
    if (argbOut && width > 0 && height > 0) {
        ...
        int needed = width * height;
        if (len >= needed) {
            // BUILD2RN: mobile video fast path. Do not allocate a std::vector every frame and do not copy twice.
...
// BUILD2RN REAL CORE ADAPTER SLOT
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_shutdown(JNIEnv* env, jclass) {
    ...
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RN_QT_CPP_ONLY worker=STOPPED audio=CLEARED input=RELEASED");
#else
    for (int i = 0; i < 8; ++i) g_input[i] = false;
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_RN_QT_CPP_ONLY noVendor input=RELEASED");
#endif
}
