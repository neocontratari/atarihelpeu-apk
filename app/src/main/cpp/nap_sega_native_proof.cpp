...
// BUILD2QU: C++ only + lower latency FM/PSG clean audio.
...
static const size_t NAP_AUDIO_TARGET_FIFO = 1536;      // BUILD2QU: QO latency target; QT zvuk byl OK, ale 0.8s pozde.
static const size_t NAP_AUDIO_MAX_FIFO = 3072;         // BUILD2QU: tvrdsi cap proti zpozdeni, jeste ne QQ-small.
static const size_t NAP_AUDIO_DESYNC_LIMIT = 512;      // BUILD2QU: FM/PSG alignment bliz obrazu.
...
static void nap_audio_latency_trim_locked() {
    const size_t margin = 320;
    while (g_audio_fm_fifo.size() > NAP_AUDIO_TARGET_FIFO + margin && g_audio_psg_fifo.size() > NAP_AUDIO_TARGET_FIFO + margin) {
        g_audio_fm_fifo.pop_front();
        g_audio_psg_fifo.pop_front();
        g_audio_drop_count += 2;
    }
    while (g_audio_fm_fifo.size() > NAP_AUDIO_MAX_FIFO) { g_audio_fm_fifo.pop_front(); g_audio_drop_count++; }
    while (g_audio_psg_fifo.size() > NAP_AUDIO_MAX_FIFO) { g_audio_psg_fifo.pop_front(); g_audio_drop_count++; }
}
...
static std::string nap_audio_status_locked() {
    ...
    out << "audio_mode=FM_PSG_QO_LATENCY_FRESH_VIDEO_QU sonic_main target=" << NAP_AUDIO_TARGET_FIFO
...
// BUILD2QU: native signal guard for real-core bring-up.
// User reported full app process crash after ROM selection in BUILD2QU/QC. Java try/catch cannot catch SIGSEGV/SIGABRT.
...
#if NAP_SEGA_VENDOR_CORE_PRESENT
// BUILD2QU: tiny Android frontend for the real ClownMDEmu-core.
...
static int g_real_step_stage = 0; // BUILD2QU: explicit core step: 0 constant, 1 init, 2 setcart, 3 hardreset, 4 iterate
...
// BUILD2QU: reset the huge core struct in-place. Do NOT use `g_real = NapRealCoreState()`: \n// ClownMDEmu is >1 MB and a temporary can overflow Android/WebView thread stack.
...
// BUILD2QU: forward declaration required by C++ before nap_real_load_rom_bytes().
...
static void nap_audio_fm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_fm_acc >= srcRate) {
            ...
            // BUILD2QU: leave headroom for a light PSG path; avoid 2QL clipping/chrceni.
...
static void nap_audio_psg(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (g_audio_psg_acc >= srcRate) {
            ...
            // BUILD2QU: Sonic jump/ring effects live here. Keep audible, but much softer than QL to stop crackle.
...
static void nap_audio_pcm(void*, ClownMDEmu *c, size_t n, void (*gen)(ClownMDEmu*, cc_s16l*, size_t)) {
    // BUILD2QU: PCM is diagnostic only for now. QL mixed PCM/CDDA and made audio strange/crackly.
...
static std::string nap_real_load_rom_bytes(const uint8_t* bytes, size_t size) {
    ...
    // BUILD2QU: do not run ClownMDEmu from the WebView JavaBridge thread or from View.onDraw.
    ...
    // BUILD2QU: ClownMDEmu core state is large and the emulator core can use more native stack than Android's default.
    ...
    out << "audio=FM_PSG_QO_LATENCY_FRESH_VIDEO_QU to Java AudioTrack; Java wrapper disabled; QO-latency profile; audio starts only after first rendered frame";
    return out.str();
}
...
static std::string nap_real_step_once() {
    std::lock_guard<std::mutex> lock(g_real_mutex);
    std::ostringstream out;
    out << "REAL_CORE_STEP_DISABLED_IN_BUILD2QU\n";
...
static bool nap_real_render_to_argb(int out_w, int out_h, jint *out_px) {
    ...
    // BUILD2QU: never call ClownMDEmu_Iterate() from Android View.onDraw.
    ...
    const std::vector<uint32_t> &src = g_real.frame_argb;
...
static void nap_real_worker_thread(int generation) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    NAPLOG("BUILD2QU real core worker start gen=%d bigstack=8MB", generation);
    try {
        ...
        while (g_real_thread_run.load() != 0 && generation == g_real_thread_generation.load()) {
            ...
            auto iter_start = std::chrono::steady_clock::now();
            {
                ...
                if (g_real.loaded) {
                    ...
                    if ((it & 31u) == 0u) {
                        g_real.status = "REAL_CORE_RENDER_OK_WORKER_THREAD_QU frameCounter=" + std::to_string(g_real.frame_counter) +
    ...
    g_real_thread_alive.store(0);
    NAPLOG("BUILD2QU real core worker stop gen=%d", generation);
#endif
}
...
static void nap_render_guard_frame(int width, int height, jintArray argbOut, JNIEnv* env) {
    ...
    // BUILD2QU: blank guarded monitor. No running cubes, no center square, no hash bars.
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeCoreBuildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2QU NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    "JNI bridge: OK\n"
                    ...
                    "Status: C++ only Sega runtime; Java wrapper disabled; QP-clean FM/PSG audio restored + render watchdog; no ROM in APK";
...
Java_eu_atarihelp_emu10_NativeSegaProofActivity_nativeRomInfo(JNIEnv* env, jclass, jbyteArray romBytes) {
    ...
    out << "BUILD2QU ma vypnuty proof pattern a chrani real-core load proti padu aplikace.\n";
    out << "Dalsi krok je podle logu opravit konkretni core init/reset/iterate misto dalsich fake patternu.\n";
    std::string s = out.str();
    NAPLOG("BUILD2QU ROM info generated, bytes=%d fnv=0x%08x", len, fnv);
    return env->NewStringUTF(s.c_str());
}
...
// BUILD2QU: same native core exposed to MainActivity/WebView in-place bridge.
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_buildString(JNIEnv* env, jclass) {
    std::string s = "BUILD2QU NATIVE C++ REAL CORE BIGSTACK THREAD STACK FIX OK\n"
                    ...
                    "C++ PCM audio generator: OK\n"
                    "C++ guarded render: OK\n"
                    "Status: normal Sega UI; Java wrapper disabled; QP-clean FM/PSG audio restored + native render watchdog against sound-only black screen; no ROM in APK";
    return env->NewStringUTF(s.c_str());
}
...
// BUILD2QU REAL CORE ADAPTER SLOT
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_realCoreStatus(JNIEnv* env, jclass) {
    ...
    out << "pattern=OFF no running cubes/no squares; cached real frame path active\n";
    {
        ...
        out << "frameSize=" << g_real.frame_w << "x" << g_real.frame_h << "\n";
...
extern "C" JNIEXPORT jstring JNICALL
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_clearAudioQueue(JNIEnv* env, jclass) {
#if NAP_SEGA_VENDOR_CORE_PRESENT
    nap_audio_clear();
    return env->NewStringUTF("AUDIO_QUEUE_CLEARED_QU");
#else
    return env->NewStringUTF("AUDIO_QUEUE_CLEAR_NO_VENDOR_QU");
#endif
}
...
Java_eu_atarihelp_emu10_NativeSegaCoreBridge_shutdown(JNIEnv* env, jclass) {
    ...
    nap_audio_clear();
    {
        ...
        g_real.loaded = false;
        g_real.frame_ready = false;
        g_real.status = "REAL_CORE_STOPPED_BY_LIFECYCLE_QU_CPP_ONLY";
    }
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_QU_CPP_ONLY worker=STOPPED audio=CLEARED input=RELEASED");
#else
    for (int i = 0; i < 8; ++i) g_input[i] = false;
    return env->NewStringUTF("NATIVE_SHUTDOWN_OK_QU_CPP_ONLY noVendor input=RELEASED");
#endif
}
