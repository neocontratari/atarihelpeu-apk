...
#define JOYP(port)                                                                                                \
   ...
   if (info == NULL || info->path == NULL || info->path[0] == '\0')
   {
      ...
      /* Region A TYP musi sedet dohromady - typ nesmi zustat na necem jinem,
         jinak casovani bezi proti sobe.
         POZOR NA REGION: rozhoduje, KTERY SOUBOR BIOSU se nahraje
         (psxMemReset() indexuje Config.Bios[] prave timhle). EU = scph7502,
         US = scph5501/1001. V B49 to bylo prehozeno na US, ale merení,
         o ktere se to oprelo (946 z 1200 snimku s obrazem), bylo udelano
         na EVROPSKEM BIOSu. Prehozeni tedy nic nespravilo - jen zmenilo
         obrazovku, kterou ladime, a 50 Hz na 60 Hz.
         Prememereno na jadre mimo telefon (1200 snimku, start bez disku):
           EU + PAL + SetCdOpenCaseTime(-1) -> 946 snimku s obrazem
           US + NTSC + SetCdOpenCaseTime(-1) -> 890 snimku s obrazem
           EU + PAL BEZ SetCdOpenCaseTime    ->   0 snimku s obrazem
         Rozhoduje tedy SetCdOpenCaseTime, ne region. Vracim EU/PAL. */
      Config.PsxRegion = PSX_REGION_EU;
