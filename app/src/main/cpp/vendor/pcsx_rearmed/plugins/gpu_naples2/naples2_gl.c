...
static struct {
    ...
    GLuint   tex_vram;          /* Textura, ze ktere se texturuje: SYROVA VRAM
                                   jadra, 2 bajty na texel (GL_LUMINANCE_ALPHA).
                                   Plni ji n2_upload_all_vram() primym prenosem
                                   z gpu.vram. Jsou v ni i PALETY (u BIOSu na
                                   640-768,384) - proto se nesmi nahradit kopii
                                   obrazu, jak jsem to zkusil v B43: palety tam
                                   nejsou a ctyrbitove textury se pak prelozily
                                   na cerne barvy (tvar zustal, barva zmizela).
                                   Nikdy k ni nepripojovat ramec - viz n2_init(). */
...
int n2_init(void)
{
    ...
    /* POZOR - SEM UZ NIKDY NEDAVAT RAMEC (FBO).
       V B43 byla tex_vram docasne RGBA a visel na ni ramec 'fbo_vram', aby se
       do ni dala nakreslit kopie obrazu. V B46 se textura vratila zpatky na
       GL_LUMINANCE_ALPHA (kvuli paletam), ale ramec u ni zustal. Do textury
       formatu LUMINANCE_ALPHA ale GLES2 kreslit NESMI - neni to barevne
       vykreslitelny format. Ramec byl proto neuplny
       (GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT = 0x8cd6), n2_init() skoncil
       chybou hned tady a NENAKRESLILO SE VUBEC NIC - ani BIOS, ani hry.
       tex_vram se plni prenosem z pameti jadra (n2_upload_all_vram), zadny
       ramec k tomu neni potreba. */
