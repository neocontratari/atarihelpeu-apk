# B132 — ZVUK V INTRU A ATARI S PUŠTĚNOU TV (versionCode 180)
...
## Deváté kolo — zvuk v intru a kousání Atari s TV

### 1) Atari se kousalo s puštěnou WEB TV

**Ta větev pro tempo snímání byla obrácená.** Našel jsem to v logu:

```
TV bere z Atari ~29 snímků za vteřinu
jeden snímek stojí ~29 ms PixelCopy + ~9 ms kreslení H264
29 × 38 ms = 1102 ms na jednu vteřinu
```

Víc, než má hlavní vlákno k dispozici. A Atari se emuluje **v JavaScriptu
na tom samém hlavním vlákně**, takže mu nezbyl čas — kousal se obraz
i zvuk, na mobilu i na TV.

Proč to tak bylo: PS1 a Sega dávají snímek **přímo z jádra**, snímání
u nich nic nestojí, a tak mají rychlou větev právem. Atari se ale musí
snímat `PixelCopy` z celého okna — a spadlo do větve označené
`Sega/Atari: rychleji`.

Ta větev byla psaná v době, kdy se Sega ještě snímala z okna. **Od B117
Sega chodí z jádra a v té větvi zůstalo jen Atari** — tedy přesně to,
co tam patří nejméně.

```java
effectiveDelay = ps1PrimoZJadra
        ? napTvWebH264FastTickMs                 // PS1/Sega: z jádra, 16 ms stačí
        : naAtariJs
            ? Math.max(napTvWebFrameDelayMs, 66) // Atari: hlavní vlákno musí dýchat
            : Math.max(8, napTvWebFrameDelayMs / 2);
```

**66 ms = asi 15 snímků za vteřinu na TV.** Je to méně plynulá TV, ale
emulátor běží. Je to jediné číslo toho kompromisu — kdyby to bylo na TV
moc trhané, řekni a snížím ho.

### 2) Zvuk v intru

WebView má `setMediaPlaybackRequiresUserGesture(false)`, takže to nebylo
tím. Prohlížeč ale často vytvoří `AudioContext` **uspaný** a pustí ho až
po doteku obrazovky — a intro běží hned po startu, kdy se nikdo ničeho
nedotkne.

Měl jsem tam jen jeden pokus o probuzení hned na začátku. Teď se zvuk
**budí opakovaně**, dokud neběží — stojí to jedno porovnání na snímek.

A hlavně: **stav zvuku se hlásí do logu.** Příště nebudeme hádat.

```
BUILD2SA24 INTRO_ZVUK vytvoren, stav=suspended
BUILD2SA24 INTRO_ZVUK BEZI
```

Ověřeno proti napodobenině prohlížeče, který se probudí až na třetí
pokus: 3 pokusy, na konci běží, 499 tónů zahráno.

**Jestli ani teď zvuk nepůjde**, v logu bude vidět proč — buď
`AudioContext v prohlizeci NENI`, nebo `probuzeni selhalo`, nebo
zůstane `stav=suspended` bez `BEZI`.
